 ...
 */
public class FluxnovaMapImpl extends BpmnModelElementInstanceImpl implements FluxnovaMap {

  protected static ChildElementCollection<FluxnovaEntry> camundaEntryCollection;

  public static void registerType(ModelBuilder modelBuilder) {
    ModelElementTypeBuilder typeBuilder = modelBuilder.defineType(FluxnovaMap.class, BpmnModelConstants.CAMUNDA_ELEMENT_MAP)
      .namespaceUri(CAMUNDA_NS)
      .instanceProvider(new ModelTypeInstanceProvider<FluxnovaMap>() {
        public FluxnovaMap newInstance(ModelTypeInstanceContext instanceContext) {
          return new FluxnovaMapImpl(instanceContext);
        }
      });

    SequenceBuilder sequenceBuilder = typeBuilder.sequence();

    camundaEntryCollection = sequenceBuilder.elementCollection(FluxnovaEntry.class)
      .build();

    typeBuilder.build();
  }

  public FluxnovaMapImpl(ModelTypeInstanceContext instanceContext) {
    super(instanceContext);
  }

  public Collection<FluxnovaEntry> getFluxnovaEntries() {
