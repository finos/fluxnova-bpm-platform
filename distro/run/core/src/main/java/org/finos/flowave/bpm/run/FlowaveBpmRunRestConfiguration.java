...
import org.finos.flowave.bpm.spring.boot.starter.rest.FlowaveBpmRestInitializer;
...
@ConditionalOnClass(FlowaveBpmRestInitializer.class)
public class FlowaveBpmRunRestConfiguration {

  @Autowired
  FlowaveBpmRunProperties camundaBpmRunProperties;
  ...
  @Bean
  @ConditionalOnProperty(name = "enabled", havingValue = "true", prefix = FlowaveBpmRunAuthenticationProperties.PREFIX)
  public FilterRegistrationBean<Filter> processEngineAuthenticationFilter(JerseyApplicationPath applicationPath) {
    ...
    FlowaveBpmRunAuthenticationProperties properties = camundaBpmRunProperties.getAuth();
    if (properties.getAuthentication() == null || FlowaveBpmRunAuthenticationProperties.DEFAULT_AUTH.equals(properties.getAuthentication())) {
  ...
  @Bean
  @ConditionalOnProperty(name = "enabled", havingValue = "true", prefix = FlowaveBpmRunCorsProperty.PREFIX)
  public FilterRegistrationBean<Filter> corsFilter(JerseyApplicationPath applicationPath) {
    ...
    registration.addInitParameter(CorsFilter.PARAM_CORS_ALLOWED_METHODS,
                                  FlowaveBpmRunCorsProperty.DEFAULT_HTTP_METHODS);
