 ...
 * @augments {FxnSDK.form.AbstractFormField}
 */
var ChoicesFieldHandler = AbstractFormField.extend(
  /** @lends FxnSDK.form.ChoicesFieldHandler.prototype */
  {
    /**
     ...
     *
     * @return {FxnSDK.form.ChoicesFieldHandler} Chainable method.
     */
    applyValue: function() {
      var selectedIndex = this.element[0].selectedIndex;
      // if fxn-choices variable is defined, apply options
      ...
      // select option referenced in fxn-variable-name (if any)
      ...
      if (variableValue !== this.previousValue) {
        ...
        this.element.trigger('fxnFormVariableApplied', variableValue);
  ...
  /** @lends FxnSDK.form.ChoicesFieldHandler */
  {
    selector: 'select[' + constants.DIRECTIVE_FXN_VARIABLE_NAME + ']'
  }
);
