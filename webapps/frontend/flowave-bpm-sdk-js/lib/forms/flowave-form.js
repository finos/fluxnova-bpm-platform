 ...
 */
function FlowaveForm(options) {
  if (!options) {
    throw new Error('FlowaveForm need to be initialized with options.');
  }
  ...
  if (options.client) {
    ...
    this.client = new FwSDK.Client(options.clientConfig || {});
  }
  ...
  if (!this.formElement && !this.containerElement) {
    return done(
      new Error(
        "FlowaveForm needs to be initilized with either 'formElement' or 'containerElement'"
      )
    );
  }
...
/**
 * @memberof FwSDK.form.FlowaveForm.prototype
 */
FlowaveForm.prototype.initializeHandler = function(FieldHandler) {
  var self = this;
 ...
 * @memberof FwSDK.form.FlowaveForm.prototype
 */
FlowaveForm.prototype.initialize = function(done) {
  done =
    done ||
    function(err) {
      if (err) throw err;
 ...
 * @memberof FwSDK.form.FlowaveForm.prototype
 */
FlowaveForm.prototype.renderForm = function(formHtmlSource) {
 ...
 * @memberof FwSDK.form.FlowaveForm.prototype
 */
FlowaveForm.prototype.initializeForm = function(done) {
  var self = this;
...
FlowaveForm.prototype.initializeFieldHandlers = function() {
 ...
 * @memberof FwSDK.form.FlowaveForm.prototype
 */
FlowaveForm.prototype.initializeFormScripts = function() {
  var formScriptElements = $(
    'script[' + constants.DIRECTIVE_FW_SCRIPT + ']',
    this.formElement
  );
...
FlowaveForm.prototype.executeFormScripts = function() {
  for (var i = 0; i < this.scripts.length; i++) {
    this.executeFormScript(this.scripts[i]);
  }
};

FlowaveForm.prototype.executeFormScript = function(script) {
  /*eslint-disable */
  /* jshint unused: false */
  (function(fwForm) {
 ...
 * something truthy.
 */
FlowaveForm.prototype.store = function(callback) {
  ...
  try {
    ...
    // store it
    localStorage.setItem('fwForm:' + formId, JSON.stringify(store));
  } catch (error) {
    if (typeof callback === 'function') {
 ...
 */
FlowaveForm.prototype.isRestorable = function() {
  var formId = this.taskId || this.processDefinitionId || this.caseInstanceId;
  ...
  if (!localStorage.getItem('fwForm:' + formId)) {
    return false;
  }

  // unserialize
  var stored = localStorage.getItem('fwForm:' + formId);
  try {
 ...
 * something truthy.
 */
FlowaveForm.prototype.restore = function(callback) {
  var stored;
  ...
  try {
    ...
    stored = localStorage.getItem('fwForm:' + formId);
 ...
 * @memberof FwSDK.form.FlowaveForm.prototype
 */
FlowaveForm.prototype.submit = function(callback) {
  ...
  if (this.submitPrevented) {
    var err = new Error('fwForm submission prevented');
  ...
  this.transformFiles(function() {
    ...
    self.submitVariables(function(err, result) {
      ...
      localStorage.removeItem('fwForm:' + formId);
 ...
 * @memberof FwSDK.form.FlowaveForm.prototype
 */
FlowaveForm.prototype.error = function(errorCode, errorMessage, callback) {
  var formId = this.taskId || this.processDefinitionId;

  this.errorPrevented = false;
  this.fireEvent('error');
  if (this.errorPrevented) {
    var err = new Error('fwForm error prevented');
 ...
 * @memberof FwSDK.form.FlowaveForm.prototype
 */
FlowaveForm.prototype.escalate = function(escalationCode, callback) {
  var formId = this.taskId || this.processDefinitionId;

  this.escalationPrevented = false;
  this.fireEvent('escalation');
  if (this.escalationPrevented) {
    var err = new Error('fwForm escalation prevented');
...
FlowaveForm.prototype.transformFiles = function(callback) {
  var that = this;
  var counter = 1;
    ...
    if (element.getAttribute('type') === 'file') {
      ...
      if (typeof FileReader === 'function' && element.files.length > 0) {
        if (
          ...
          (parseInt(element.getAttribute('fw-max-filesize'), 10) || 5000000)
        ) {
          throw new Error(
 ...
 * @memberof FwSDK.form.FlowaveForm.prototype
 */
FlowaveForm.prototype.fetchVariables = function(done) {
  done = done || function() {};
 ...
 * @memberof FwSDK.form.FlowaveForm.prototype
 */
FlowaveForm.prototype.parseVariables = function() {
 ...
 * @memberof FwSDK.form.FlowaveForm.prototype
 */
FlowaveForm.prototype.submitVariables = function(done) {
  done = done || function() {};
  ...
  if (this.taskId) {
    ...
    var businessKey =
      this.businessKey ||
      this.formElement.find('input[type="text"][fw-business-key]').val();
    if (businessKey) {
 ...
 * @memberof FwSDK.form.FlowaveForm.prototype
 */
FlowaveForm.prototype.storeOriginalValues = function(variables) {
 ...
 * @memberof FwSDK.form.FlowaveForm.prototype
 */
FlowaveForm.prototype.mergeVariables = function(variables) {
  var vars = this.variableManager.variables;
 ...
 * @memberof FwSDK.form.FlowaveForm.prototype
 */
FlowaveForm.prototype.applyVariables = function() {
  for (var i in this.fields) {
 ...
 * @memberof FwSDK.form.FlowaveForm.prototype
 */
FlowaveForm.prototype.retrieveVariables = function() {
  for (var i in this.fields) {
 ...
 * @memberof FwSDK.form.FlowaveForm.prototype
 */
FlowaveForm.prototype.fireEvent = function(eventName, obj) {
  this.trigger(eventName, obj);
};

/**
 * @memberof FwSDK.form.FlowaveForm
 */
FlowaveForm.$ = $;

FlowaveForm.VariableManager = VariableManager;
FlowaveForm.fields = {};
FlowaveForm.fields.InputFieldHandler = InputFieldHandler;
FlowaveForm.fields.ChoicesFieldHandler = ChoicesFieldHandler;

/**
 * @memberof FwSDK.form.FlowaveForm
 */
FlowaveForm.cleanLocalStorage = function(timestamp) {
  for (var i = 0; i < localStorage.length; i++) {
    var key = localStorage.key(i);
    if (key.indexOf('fwForm:') === 0) {
...
/**
 * @memberof FwSDK.form.FlowaveForm
 * @borrows FwSDK.BaseClass.extend as extend
 * @name extend
 * @type {Function}
 */
FlowaveForm.extend = BaseClass.extend;

module.exports = FlowaveForm;
