...
/* global jQuery: false */
'use strict';
var FwSDK = require('../../../lib/angularjs/index.js');
...
describe('The input field', function() {
  ...
  it('prepares the testing environemnt', function(done) {
    jQuery.ajax('/base/test/karma/forms-angularjs/angular-form.html', {
      success: function(data) {
        ...
        expect(typeof FwSDK).to.eql('object');
  ...
  it('needs a process definition', function(done) {
    ...
    fwClient.resource('process-definition').list({}, function(err, result) {
  ...
  it('should provide FwSDKFormsAngular', function() {
    expect(FwSDK.Form).to.not.be.undefined;
  });
  ...
  it('should use pre-rendered form', function(done) {
    ...
    function whenRetrieved(err) {
      ...
      fwForm.variableManager.variable('stringVar').value = 'secondUpdate';
    ...
    function ready(err) {
      ...
      fwForm.retrieveVariables();

      waitUntil(function() {
        // the variable updated using angular is updated in the variable manager
        return 'updated' === fwForm.variableManager.variable('stringVar').value;
      }, whenRetrieved);
    }
  ...
  it('should render form', function(done) {
    ...
    function whenUpdated(err) {
      ...
      fwForm.variableManager.variable('autoBindVar').value = 'autoBindValue';
    ...
    function ready(err) {
      ...
      waitUntil(function() {
        ...
        return $el.hasClass('ng-invalid') &&
                $el.hasClass('ng-invalid-fw-variable-type') &&
                scope.form &&
                scope.form.$invalid &&
                scope.form.$error &&
                scope.form.$error.fwVariableType;
      }, done);
    }
