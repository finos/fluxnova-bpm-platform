...
describe('The form', function() {
  ...
  it('initialize', function(done) {
    function ready(err) {
      ...
      var customVar = fwForm.variableManager.variable('customVar');
      expect(customVar.name).to.eql('customVar');
      expect(customVar.type).to.eql('String');

      // expect form field to be populated
      expect($('#customField', fwForm.formElement).val()).to.eql('someValue');
      ...
      expect(fwForm.submitPrevented).to.be.ok;

      // if we change the value of the form field
      $('#customField', fwForm.formElement).val('updated');

      // and re-attempt submit
      fwForm.submit();

      // we expect the submit to pass
      expect(fwForm.submitPrevented).to.not.be.ok;

      done();
    }

    var fwForm = new FwSDK.Form({
      client: fwClient,
