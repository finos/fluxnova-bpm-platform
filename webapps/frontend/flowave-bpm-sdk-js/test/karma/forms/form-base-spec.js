...
describe('The form', function() {
  ...
  it('gets the process definition with a promise', function() {
    return fwClient.resource('process-definition').list({}).then(
      function(result) {
        procDef = result.items.pop();
        expect(procDef.id).to.be.ok;
      }
  ...
  it('exists globally', function() {
    expect(FwSDK.Form).to.be.a('function');
  });


  it('has a DOM library', function() {
    expect(FwSDK.Form.$).to.be.ok;
  });


  it('initialize', function(done) {
    expect(fwClient).to.be.ok;
    ...
    function initialized() {
      ...
      expect(fwForm.formFieldHandlers).to.be.an('array');

      expect(fwForm.fields).to.be.an('array');
  ...
  describe('choices field', function() {
    ...
    describe('single choice', function() {
      var $select;
      beforeEach(function() {
        $select = $simpleFormDoc.find('select[fw-variable-name]:not([multiple])');
      });
    ...
    describe('multiple choices', function() {
      var $select;
      before(function() {
        $select = $simpleFormDoc.find('select[fw-variable-name][multiple]');
      });
