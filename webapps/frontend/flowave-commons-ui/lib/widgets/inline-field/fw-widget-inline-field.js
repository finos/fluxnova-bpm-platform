...
var angular = require('flowave-bpm-sdk-js/vendor/angular'),
  $ = require('jquery'),
  template = require('./fw-widget-inline-field.html?raw');
    ...
    return {
      ...
      link: function(scope, element) {
        ...
        scope.selectNextInlineField = function(reversed) {
          var allFields = $(
            "[fw-widget-inline-field][type='text'], [fw-widget-inline-field][type='option']"
          );
          for (
        ...
        scope.trapKeyboard = function(evt, reverse) {
          if (isDate() && evt.keyCode === 9) {
            ...
            if (!reverse && !evt.shiftKey) {
              ...
              $(
                '.fw-widget-inline-field.field-control > .datepicker > table'
              )[0].focus();
              evt.preventDefault();
            } else if (reverse && evt.shiftKey) {
              // focus the cancel button
              $(
                '.fw-widget-inline-field.btn-group > button[ng-click="cancelChange($event)"]'
              )[0].focus();
              evt.preventDefault();
            }
          }
