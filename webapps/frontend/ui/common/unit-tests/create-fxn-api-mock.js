...
/**
 * Creates mocked fxnAPI instance, that has resource method that always returns fakeResource property of mocked fxnAPI.
 *
 * @param resourceMethods   methods that should be mocked on resource.
 * @returns {{resource: *, fakeResource: *}}
 */
function createFxnApiMock(resourceMethods) {
  ...
  fxnAPI.resource.returns(fakeResource);

  return fxnAPI;
}

createFxnApiMock.defaultResourceMethods = defaultResourceMethods.slice(); // Copy for safety

module.exports = createFxnApiMock;
