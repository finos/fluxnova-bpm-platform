...
/**
 * Creates mocked fwAPI instance, that has resource method that always returns fakeResource property of mocked fwAPI.
 *
 * @param resourceMethods   methods that should be mocked on resource.
 * @returns {{resource: *, fakeResource: *}}
 */
function createFwApiMock(resourceMethods) {
  ...
  fwAPI.resource.returns(fakeResource);

  return fwAPI;
}

createFwApiMock.defaultResourceMethods = defaultResourceMethods.slice(); // Copy for safety

module.exports = createFwApiMock;
