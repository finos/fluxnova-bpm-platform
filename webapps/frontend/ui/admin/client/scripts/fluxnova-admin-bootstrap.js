...
// fluxnova-admin-bootstrap is copied as-is, so we have to inline everything
...
const loadConfig = (async function() {
  ...
  window.fxnAdminConf = config;
  return config;
})();

define('fluxnova-admin-bootstrap', function() {
  'use strict';
  const bootstrap = config => {
    ...
    var fluxnovaAdminUi = require('./fluxnova-admin-ui');
    fluxnovaAdminUi.exposePackages(window);

    requirejs([`${appRoot}/lib/globalize.js`], function(globalize) {
      globalize(
        requirejs,
        ['angular', 'fluxnova-commons-ui', 'fluxnova-bpm-sdk-js', 'jquery'],
        window
      );
      ...
      requirejs(dependencies, function(angular) {
        ...
        if (typeof config !== 'undefined' && config.requireJsConfig) {
          ...
          requirejs(custom.deps || [], function() {
            ...
            angular.module('fxn.admin.custom', custom.ngDeps);

            clearAmdGlobals();

            // now that we loaded the plugins and the additional modules, we can finally
            // initialize Admin
            fluxnovaAdminUi.init(pluginDependencies);
          });
        } else {
          // for consistency, also create a empty module
          angular.module('fxn.admin.custom', []);
...
requirejs(['fluxnova-admin-bootstrap'], function() {});
