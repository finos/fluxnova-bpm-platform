...
// flowave-admin-bootstrap is copied as-is, so we have to inline everything
...
const loadConfig = (async function() {
  ...
  window.fwAdminConf = config;
  return config;
})();

define('flowave-admin-bootstrap', function() {
  'use strict';
  const bootstrap = config => {
    ...
    var camundaAdminUi = require('./flowave-admin-ui');
    camundaAdminUi.exposePackages(window);

    requirejs([`${appRoot}/lib/globalize.js`], function(globalize) {
      globalize(
        requirejs,
        ['angular', 'flowave-commons-ui', 'flowave-bpm-sdk-js', 'jquery'],
        window
      );
...
requirejs(['flowave-admin-bootstrap'], function() {});
