...
// fluxnova-welcome-bootstrap is copied as-is, so we have to inline everything
...
const loadConfig = (async function() {
  ...
  window.fxnWelcomeConf = config;
  return config;
})();

define('fluxnova-welcome-bootstrap', function() {
  'use strict';
  const bootstrap = config => {
    ...
    var fluxnovaWelcomeUi = require('./fluxnova-welcome-ui');
    fluxnovaWelcomeUi.exposePackages(window);
      ...
      requirejs(dependencies, function(angular) {
        ...
        if (typeof config !== 'undefined' && config.requireJsConfig) {
          ...
          requirejs(custom.deps || [], function() {
            ...
            angular.module('fxn.welcome.custom', custom.ngDeps);

            clearAmdGlobals();

            // now that we loaded the plugins and the additional modules, we can finally
            // initialize Welcome
            fluxnovaWelcomeUi.init(pluginDependencies);
          });
        } else {
          // for consistency, also create a empty module
          angular.module('fxn.welcome.custom', []);
...
requirejs(['fluxnova-welcome-bootstrap'], function() {});
