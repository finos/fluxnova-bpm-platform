...
var template = require('./fxn-tasklist-sorting-choices.html?raw');
    ...
    return {
      ...
      link: function(scope, element) {
        ...
        scope.changeSorting = function(idx, id, type, value) {
          ...
          $timeout(function() {
            var element = document.querySelector(
              '[fxn-sorting-choices] li:nth-child(0n+' +
                (idx + 1) +
                ') a.dropdown-toggle'
            );
            if (element) {
        ...
        scope.addSorting = function(id, type, value) {
          ...
          $timeout(function() {
            var element = document.querySelector(
              '[fxn-sorting-choices] li:last-child a.dropdown-toggle'
            );
            if (element) {
              element.focus();
            }
          ...
          $timeout(function() {
            ...
            if (scope.sortings.length !== index) {
              element = document.querySelector(
                '[fxn-sorting-choices] li.sorting-choice:nth-child(0n+' +
                  (index + 1) +
                  ') a:first-child'
              );
            } else {
              element = document.querySelector(
                '[fxn-sorting-choices] li.sorting-choice:nth-last-child(0n+2) a:first-child'
              );
            }
            if (element) {
              element.focus();
