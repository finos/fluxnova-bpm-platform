...
module.exports = [
  ...
  'Notifications',
  'unfixDate',
  function(fxnAPI, $translate, Notifications, unfixDate) {
    return {
      restrict: 'A',
      ...
      link: function($scope, $element, attrs, formController) {
        const Task = fxnAPI.resource('task');
        const ProcessDefinition = fxnAPI.resource('process-definition');
        const taskId = formController.getParams().taskId;
        const savedState = JSON.parse(
          localStorage.getItem(`fluxnova-form:${taskId}`) || '{}'
        );
        let form;
        ...
        function handleSave() {
          ...
          localStorage.setItem(`fluxnova-form:${taskId}`, JSON.stringify(data));
          formController.notifyFormDirty(false);
        }

        function clearSave() {
          localStorage.removeItem(`fluxnova-form:${taskId}`);
        }
