...
//  fluxnova-tasklist-bootstrap is copied as-is, so we have to inline everything
...
const loadConfig = (async function() {
  ...
  window.fxnTasklistConf = config;
  return config;
})();

define('fluxnova-tasklist-bootstrap', function() {
  'use strict';
  const bootstrap = config => {
    ...
    var fluxnovaTasklistUi = require('./fluxnova-tasklist-ui');
    fluxnovaTasklistUi.exposePackages(window);
      ...
      requirejs(dependencies, function(angular) {
        ...
        if (typeof config !== 'undefined' && config.requireJsConfig) {
          ...
          requirejs(custom.deps || [], function() {
            ...
            angular.module('fxn.tasklist.custom', custom.ngDeps);

            clearAmdGlobals();

            // now that we loaded the plugins and the additional modules, we can finally
            // initialize the tasklist
            fluxnovaTasklistUi.init(pluginDependencies);
          });
        } else {
          // for consistency, also create a empty module
          angular.module('fxn.tasklist.custom', []);
...
requirejs(['fluxnova-tasklist-bootstrap'], function() {});
